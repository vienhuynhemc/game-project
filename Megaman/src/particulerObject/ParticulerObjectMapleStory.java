...
public abstract class ParticulerObjectMapleStory extends GameObjectMapleStory implements Upload, Draw {
	...
	public static final int SWITCHMEGAMAN =5;
	...
	public ParticulerObjectMapleStory(float x, float y, GameWorldMapleStory game, float mass, float width, float height,
			int blood, int dame) {
		super(x, y, game);
		this.mass = mass;
		this.dame = dame;
	...
	public void upload() {
		...
		case ALIVE:
			...
			if (getBlood() <= 0) {
				getToDeath();
				if (this.getClass() != InnerRage.class) {
					setStage(DEATH);
				}
			}
			break;
