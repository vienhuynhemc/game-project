...
public class WhenMettalTDeath extends WhenDeath {
	...
	public WhenMettalTDeath(float x, float y, GameWorldMegamanX game) {
		super(x, y, game, 0.1f);
		...
		Rectangle rect1 = new Rectangle((int) x, (int) y, 20, 16);
		Rectangle rect2 = new Rectangle((int) x, (int) y, 26, 24);
		Rectangle rect3 = new Rectangle((int) x, (int) y, 30, 26);
		Rectangle rect4 = new Rectangle((int) x, (int) y, 38, 42);
		Rectangle rect5 = new Rectangle((int) x, (int) y, 40, 32);

		getListRectangle().add(rect1);
		getListRectangle().add(rect2);
		getListRectangle().add(rect3);
		getListRectangle().add(rect4);
		getListRectangle().add(rect5);
		...
		getListStage().put(4, ParticulerObjectMegamanX.ALIVE);

		animation1 = new Animation(getGame().getInputData().getListAnimation().get("mettalDead1"));
		animation2 = new Animation(getGame().getInputData().getListAnimation().get("mettalDead2"));
		animation3 = new Animation(getGame().getInputData().getListAnimation().get("mettalDead3"));
		animation4 = new Animation(getGame().getInputData().getListAnimation().get("mettalDead4"));
		animation5 = new Animation(getGame().getInputData().getListAnimation().get("mettalDead5"));
	...
	public void draw(Graphics2D g2) {
		...
		if (!getListIgnoreDrawRect().get(4)) {
			if (getListStage().get(4) == ParticulerObjectMegamanX.FEY && System.nanoTime() % 3 == 2) {
			} else {
				animation4.upload(System.nanoTime());
				animation4.draw(g2, (int) (getListRectangle().get(3).getX() - getGame().getCamera().getX()),
						(int) (getListRectangle().get(0).getY() - getGame().getCamera().getY()));
