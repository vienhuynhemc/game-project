...
public class BlackWyvern extends HumanMapleStory {
	...
	public BlackWyvern(float x, float y, GameWorldMapleStory game, int dir) {
		super(x, y, game, 0f, 314, 322, 900, 10);

		hb = new HealBarEnemies("Black Wyvern", 900, 900, getX() + 80 - getGame().getCamera().getX(),
				getY() - getGame().getCamera().getY() + 120, 170, 30);
		...
		iL = new Animation(getGame().getInputData().getListAnimation().get("bwi"));
		iR = new Animation(getGame().getInputData().getListAnimation().get("bwi"));
		iR.daoNguoc();

		bL = new Animation(getGame().getInputData().getListAnimation().get("bwbehurt"));
		bR = new Animation(getGame().getInputData().getListAnimation().get("bwbehurt"));
		bR.daoNguoc();

		aL = new Animation(getGame().getInputData().getListAnimation().get("bwa"));
		aR = new Animation(getGame().getInputData().getListAnimation().get("bwa"));
		aR.daoNguoc();

		deadL = new Animation(getGame().getInputData().getListAnimation().get("bwd"));
		deadR = new Animation(getGame().getInputData().getListAnimation().get("bwd"));
		deadR.daoNguoc();
	...
	public void upload() {
		...
		hb.upload("Black Wyvern", 900, getBlood(), getX() + 80 - getGame().getCamera().getX(),
				getY() + 120 - getGame().getCamera().getY());
	...
	public void draw(Graphics2D g2) {
		if (getStage() == BEHURT) {
			...
			if (arrayAction[currentAction].equals("NONE") || arrayAction[currentAction].equals("RUN")) {
				...
				if (getDirector() == DIR_LEFT) {
					...
					if (aL.getCurrentFrame() == 6) {
						if (count == 0) {
							count++;
							getGame().getListBullet().addObject(new BulletBlackWyvern(getGame().getMario().getX() - 220,
									getGame().getMario().getY() - 360, getGame(), getDirector()));
							ManagerSound.getInstance().getListSound().get("blackWyvern").start();
							try {
								ManagerSound.getInstance().khoiTaoHieuUng("blackWyvern");
					...
					if (aR.getCurrentFrame() == 6) {
						if (count == 0) {
							getGame().getListBullet().addObject(new BulletBlackWyvern(getGame().getMario().getX() - 300,
									getGame().getMario().getY() - 360, getGame(), getDirector()));
							count++;
							ManagerSound.getInstance().getListSound().get("blackWyvern").start();
							try {
	...
	public Rectangle getRectangleXuLiVaCham() {
		...
		return new Rectangle(rect.x + 80, rect.y + 140, rect.width - 140, rect.height - 200);
	}
