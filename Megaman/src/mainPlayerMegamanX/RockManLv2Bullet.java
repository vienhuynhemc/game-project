...
public class RockManLv2Bullet extends BulletMegamanX {

	private Animation banLeft, banRight, removeLeft, removeRight;

	public RockManLv2Bullet(float x, float y, GameWorldMegamanX game) {
		super(x, y, game, 0.1f, 54, 36, 1000, 60);
		// dame cũ là 30

		banLeft = new Animation(getGame().getInputData().getListAnimation().get("bulletRockManLv2"));
		banRight = new Animation(getGame().getInputData().getListAnimation().get("bulletRockManLv2"));
		banLeft.daoNguoc();
		removeLeft = new Animation(getGame().getInputData().getListAnimation().get("bulletRockManLv2Remove"));
		removeLeft.daoNguoc();
		removeRight = new Animation(getGame().getInputData().getListAnimation().get("bulletRockManLv2Remove"));
	...
	public void draw(Graphics2D g2) {
		...
		if (isChamTuong()) {
			...
			if (getDirector() == DIR_RIGHT) {
				...
				removeRight.draw(g2, (int) (getX() - getGame().getCamera().getX() + 50),
				...
				removeLeft.draw(g2, (int) (getX() - getGame().getCamera().getX() - 50),
						(int) (getY() - getGame().getCamera().getY()));
