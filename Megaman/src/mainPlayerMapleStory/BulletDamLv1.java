...
public class BulletDamLv1 extends BulletMapleStory {

	private long timeStart;

	public BulletDamLv1(float x, float y, GameWorldMapleStory game, int dir) {
		super(x, y, game, 0, 60, 60, 10, 30);
		// dame cũ là 10
		setSpeedX(0);
		setSpeedY(0);
	...
	public void getToDeath() {
		...
		if (getDirector() == DIR_LEFT) {
			getGame().getListEF().addEF(new WhenBulletDamLv1Dead(getX() + 10, getY(), getGame()));
		} else {
			getGame().getListEF().addEF(new WhenBulletDamLv1Dead(getX(), getY(), getGame()));
		}
	}
