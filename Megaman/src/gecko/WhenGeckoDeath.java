...
public class WhenGeckoDeath extends WhenDeath {

	private Animation removeLeft, removeRight;;

	private int director;

	public WhenGeckoDeath(float x, float y, GameWorldMegamanX game, int director) {
		super(x, y, game, 0.1f);

		removeLeft = new Animation(getGame().getInputData().getListAnimation().get("tacKeDeath"));
		removeLeft.setIsRepeat(false);
		removeRight = new Animation(getGame().getInputData().getListAnimation().get("tacKeDeath"));
