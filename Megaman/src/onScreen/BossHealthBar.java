...
public class BossHealthBar extends GameObjectMegamanX {
	...
	public BossHealthBar(float x, float y, GameWorldMegamanX game) {
		super(x, y, game);
	...
	public void uploadListHp() {
		if (!isStart) {
			...
			if (ManagerSound.getInstance().getListSound().get("hoiHp")
					.getFramePosition() == ManagerSound.getInstance().getListSound().get("hoiHp")
							.getFrameLength()) {
				try {
